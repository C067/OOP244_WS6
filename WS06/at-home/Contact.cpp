...
namespace sict {
	...
	Contact::Contact(const Contact &contact) {
		if (!isEmpty()) {
			...
			if (contact.NOP > 0) {
				...
				for (int cnt = 0; cnt < NOP; cnt++) {
					phoneNumber[cnt] = contact.phoneNumber[cnt];
				}
			}
			else {
				phoneNumber = nullptr;
	...
	Contact& Contact::operator+=(const long long number) {
		...
		if (n_size == true) {
			long long *n_Array = new long long [NOP + 1];

			for (int cnt = 0; cnt < NOP; cnt++) {
				n_Array[cnt] = phoneNumber[cnt];
			}
			NOP += 1;
			...
			for (int cnt = 0; cnt < (NOP+1); cnt++) {
				phoneNumber[cnt] = n_Array[cnt];
			}

			phoneNumber[NOP - 1] = number;
		}
